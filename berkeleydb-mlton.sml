...
in
  ...
  fun db_put (db, txnid, key, key_len, data, data_len, flags) =
      db_put_ffi (db, pOrNull txnid, key, Word32.fromInt key_len, data, Word32.fromInt data_len, Word32.fromInt flags)
  ...
  fun db_put_recno (db, txnid, key, data, data_len, flags) =
    let
      ...
      val r = db_put_recno_ffi (db, pOrNull txnid, key', data, Word32.fromInt data_len, Word32.fromInt flags)
    in
  ...
  fun db_get (db, txnid, key, key_len, flags) =
    let
      ...
      val r  = db_get_ffi (db, pOrNull txnid, key, Word32.fromInt key_len, data_r, data_len_r, Word32.fromInt flags)
    in
      if r = 0
